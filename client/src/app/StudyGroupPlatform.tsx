...
import { 
  BookOpen, Globe, ChevronDown, Database, 
...
const StudyGroupPlatform: React.FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: Event) => {
      if (showUserSwitcher && !(event.target as Element).closest('.user-switcher')) {
  ...
  }, [showUserSwitcher, showLanguageSwitcher]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center gap-2 px-3 py-2 text-sm bg-green-100 text-green-800 rounded-lg border border-green-300">
                <Database className="w-4 h-4" />
                <span>{source === 'mock' ? t('nav.mockData') : t('nav.apiData')}</span>
              </div>
