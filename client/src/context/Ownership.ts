...
/** Session-specific permissions: Admins and presenters can edit/delete sessions */
export function canPerformSessionAction(user: User | null | undefined, action: Action, session: Session): boolean {
  ...
  if (user.role === 'admin') return true;
  // Presenters can edit/delete their own sessions
  return session.presenterId === user.id && (action === 'edit' || action === 'delete');
}
